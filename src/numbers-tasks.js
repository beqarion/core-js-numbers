...
function getFibonacciNumber(index) {
  ...
  for (let i = 1; i <= index; i += 1) {
    const prevElement = fibonacciSequence[i];
    const beforePrevElement = fibonacciSequence[i - 1];

    fibonacciSequence.push(prevElement + beforePrevElement);
  }
...
function getSumOfNumbers(x1, x2, x3) {
  return (x1 * 1e10 + x2 * 1e10 + x3 * 1e10) / 1e10;
}
...
function getCountOfOddNumbers(number) {
  ...
  return positiveNumber % 2 === 1
    ? Math.trunc(positiveNumber / 2) + 1
    : Math.trunc(positiveNumber / 2);
}
